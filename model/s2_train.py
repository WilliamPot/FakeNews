# -*- coding: utf-8 -*-
"""
Created on Wed May 30 16:12:08 2018
...
def write_data(batch_size,epoch,train_data,train_label):
    for i in range(epoch):
        print('Level2 training file {} generated.'.format(i+1))
        ...
        x_pds.to_csv('train_s2/tfidf/train_data_{}.csv'.format(i+1))
        y_pds.to_csv('label_s2/tfidf/train_data_{}.csv'.format(i+1))
...
count_v1= CountVectorizer(max_features = 5000,token_pattern=r'[A-Za-z]+');
doc_sta = [" ".join(text) for text in texts_sta] 
doc_art = [" ".join(text) for text in texts_art] 
counts = count_v1.fit_transform(doc_sta+doc_art).toarray()
#counts_sta = counts[0:len(stances_doc)]
#counts_art = counts[len(stances_doc):len(counts)]
with open('Count_vectorizer.pk', 'wb') as fin:
    pkl.dump(count_v1.vocabulary_, fin)
with open('Count_vectorizer.pk', 'rb') as f:
    count_v1_voc= pkl.load(f)
count_v2= CountVectorizer(vocabulary=count_v1_voc)

#corpus = [ dictionary.doc2bow(text) for text in texts_total ]
#tfidf_model = TfidfModel(corpus)
#tfidf_model.save('stage2_tfidf_model')
#tfidf_model = TfidfModel.load('stage2_tfidf_model')
#tfidf_matrix = tfidf_model[corpus]
#tfidf_sta = tfidf_matrix[0:len(stances_doc)]
#tfidf_art = tfidf_matrix[len(stances_doc):len(counts)]
